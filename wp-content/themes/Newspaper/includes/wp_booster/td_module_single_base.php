...
//        ){
					...
					if ( preg_match( '/(<p.*>)/U', $content_part_value ) === 1 ) {
						if ( $tds_inline_ad_paragraph == $p_open_tag_count ) {
							switch ( $tds_inline_ad_align ) {
								case 'left':
									$content_buffer .= td_global_blocks::get_instance( 'td_block_ad_box' )->render( array(
										'spot_id' => 'content_inline',
										'align'   => 'left'
									) );
									break;
	 ...
	 * @return string
	 */
	function get_next_read_more_button() {
		$metavalue = get_post_meta( get_the_ID(), 'custom_source_url', true );
		$buffy     = '<div class="td-post-source-tags td-pb-padding-side" id="read_more_button">
            <a class="custom_source_url" title="Read More" target="_blank" href="' . $metavalue . '">Read More</a>
                    </div>';
	...
	function related_posts( $force_sidebar_position = '' ) {
		...
		$td_block_args = array(
			...
			//live atts - this is the default setting for this block
			'td_ajax_filter_type' => 'td_custom_related',
			//this filter type can overwrite the live filter @see
